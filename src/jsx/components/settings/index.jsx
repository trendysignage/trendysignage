...
import { getUsers, getGroups, getRoles, getDeviceProfile } from "../../../utils/api";
...
const handleDevice = (e) => {
  ...
  return (
    <>
    <AddUserModal open={showAddUserModel} setShowAddUserModel={setShowAddUserModel} setIsRefresh={setIsRefresh}/>
    <AddGroup open={showGroupModel} setShowGroupModel={setShowGroupModel} setIsRefresh={setIsRefresh}/>
    <AddDeviceProfile open={showProfileModel} setShowProfileModel={setShowProfileModel} setIsRefresh={setIsRefresh} />
      <div className="custom-content-heading d-flex flex-wrap" style={{minHeight: '600px !important'}}>
        <h1>Settings</h1>
    ...
    { dropValue && dropValue === 'Users' && <><button className="btn btn-sm btn-success" onClick={(e) => {handleUser(e)}}>Add User</button><User setIsRefresh={setIsRefresh} isRefresh={isRefresh}  users={allUsers}/> </>}
    { dropValue && dropValue === 'Groups' && <><button className="btn btn-sm btn-success" onClick={(e) => {handleGroup(e)}}>Add Groups</button><Group setIsRefresh={setIsRefresh} isRefresh={isRefresh}  groups={allGroups}/> </>}
    { dropValue && dropValue === 'Roles' && <><Roles setIsRefresh={setIsRefresh} isRefresh={isRefresh}  roles={allRoles}/> </>}
    { dropValue && dropValue === 'Device Profile' && <><button className="btn btn-sm btn-success" onClick={(e) => {handleDevice(e)}}>Add Device Profile</button><Profile allDeviceProfile={allDeviceProfile} setIsRefresh={setIsRefresh} isRefresh={isRefresh} /> </>}
